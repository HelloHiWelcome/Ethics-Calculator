...
class EthicsClassifier:
    ...
    def fit(
        ...
        """
        Train the classifier on texts, principle labels, and verdict labels.
        """
    ...
    def predict_principles(self, texts: List[str], threshold: float = 0.3) -> List[List[str]]:
        """
        Predict which principles apply for each input text.

        Returns a list of lists of principle keys.
        """
        ...
        # Try to use predict_proba if available; otherwise fall back to
        # converting decision_function scores to pseudo-probabilities.
        try:
    ...
    def predict(self, text: str, threshold: float = 0.25):
        """
        Convenience method: for a single scenario string, return

            (pred_principles, pred_verdict)

        - pred_principles: list of principle keys
        - pred_verdict: one of 'ethical', 'ambiguous', 'unethical'
        """
        if not self.is_trained:
            raise RuntimeError("Model is not trained yet!")

        # Reuse the list-based helpers so logic stays in one place
        pred_principles_list = self.predict_principles([text], threshold=threshold)
        pred_verdict_list = self.predict_verdict([text])

        pred_principles = pred_principles_list[0]
        pred_verdict = pred_verdict_list[0]
        return pred_principles, pred_verdict


def build_and_train_classifier(test_size: float = 0.3, random_state: int = 42) -> EthicsClassifier:
    """
    Build and train the classifier on TRAINING_EXAMPLES.
    Prints only a short summary (training details are hidden for end users).
    """
    ...
    # We still do a train/test split internally (you *could* add metrics later),
    # but we don't print the detailed results.
...
# Which principles we actually want to highlight as "frameworks violated"
...
# Very simple keyword heuristic for "this looks political"
POLITICAL_KEYWORDS = {
    "election", "vote", "voter", "voting", "ballot",
    "campaign", "candidate", "party", "parliament",
    "referendum", "political", "propaganda"
}


def pick_principles_for_display(text: str, predicted: list[str]) -> list[str]:
    """
    Post-process the raw predicted principles to decide what to *show*.

    - Prefer "core" ethics frameworks (fairness, democratic values, etc.)
    - For political contexts, always include democratic_values
    - Limit how many we show so it doesn't look noisy
    """
    # 1) Start with "core" principles that were predicted
    chosen = [p for p in predicted if p in CORE_DISPLAY_PRINCIPLES]

    # 2) Heuristic: if it's clearly about politics/voting/campaigns,
    #    make sure we include 'democratic_values'
    lowered = text.lower()
    if any(word in lowered for word in POLITICAL_KEYWORDS):
        if "democratic_values" not in chosen:
            chosen.insert(0, "democratic_values")

    # 3) If nothing core was chosen, fall back to whatever was predicted
    if not chosen:
        chosen = list(predicted)
    ...
    # 5) Limit how many we show (e.g., top 4)
    return unique[:4]


def classify_scenario(classifier: EthicsClassifier, text: str, threshold: float = 0.25) -> None:
    """
    Classify a scenario and print:
      - overall ethical verdict in all cases
      - if verdict is 'unethical', also print which IEEE-style principles are violated
    """
    ...
    # Only in the unethical case, show which principle frameworks are violated
    if pred_verdict == "unethical" and pred_principles:
        # Use the helper to choose which ones to actually show
        to_show = pick_principles_for_display(text, pred_principles)
        ...
        print("Violated IEEE-style principles:", ", ".join(readable))
